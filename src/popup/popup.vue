...
const saveTab = async () => {
  ...
  if (tabList.value.length > 0) {
    const findUrl = tabList.value.find((item: TabList[0]) => item.tabUrl === tabInfo.value.url);
    if (findUrl) {
...
<template>
  <main class="w-[520px] h-[400px] bg-white">
    ...
    <section class="relative">
      <button
        @click="isAddTab = !isAddTab"
        class="text-gray-500 rounded-b-md bg-white inline-block py-1 px-2 absolute top-0 right-0 shadow border-l border-r border-b"
      >
    ...
    <section class="px-4 py-10">
      <ul v-if="tabList.length > 0">
        <li v-for="item in tabList" :key="item.tabTitle" class="flex border-b py-2 px-4">
          <div v-if="editTabUrl === item.tabUrl">yes</div>
          <a
            v-else
            :href="item.tabUrl"
            target="_blank"
          ...
          <div class="ml-auto">
            <button @click="editTabUrl = item.tabUrl" class="px-1">
